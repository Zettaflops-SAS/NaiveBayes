...
<div <?php  if(session('Fallo')) { ?> class="box box-danger" <?php  }else{ ?> <?php  if(session('Alerta')) { ?> class="box box-warning" <?php  }else{ ?> class="box box-success" <?php }} ?>>
  <div class="box-header">
    <center><h3 class="box-title">Prestamos Solicitados</h3></center>
  </div>
  <div class="box-body">
    <form role="form" action="/prestamo/prestamossolicitados/consultaprestamossilicitados" method="POST">
      {{ csrf_field() }}
      <div class="row">
        <div class="col-md-2">
        ...
        <tbody>
          @foreach ($ResultadoConsultaPrestamos as $apuntador)
            <tr>
              ...
              <td class="active">
                <button type="button" class="btn btn-block btn-success btn-sm" onclick="location.href='{{ url('/algoritmo/'.$apuntador->PK.'/') }}'"><i class="fa fa-line-chart"></i>&nbsp;&nbsp;&nbsp;VER</button>
              </td>
            </tr>
          @endforeach
        </tbody>
